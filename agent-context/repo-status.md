...
Last updated: 2026-05-20T22:49:51Z
...
| Hosted recoverable-wallet smoke | Partial. On 2026-05-20, protected Supabase delivery applied the recovery-bundle schema to `CubidDev` and confirmed no pending migrations. Shell-based API smoke against the PR Passport preview is blocked by Vercel Deployment Protection returning `Authentication Required` before requests reach Passport; a Chrome-authenticated attempt against deployment `2Fp9gPxYho8vCKCE6hMRhC9Zwn75` was also blocked locally by `ERR_BLOCKED_BY_CLIENT` on the preview app and Vercel SSO URLs. Local Firebase Admin private values are blank, so user-authorized release completion could not be smoked from this checkout. |
