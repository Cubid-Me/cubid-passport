...
export default async function getOidcLoginChallenge(
  req: NextApiRequest,
  res: NextApiResponse
) {
  if (req.method !== "GET") {
  ...
  try {
    const result = await fetchOidcJson(
      `/interaction/login/${encodeURIComponent(loginChallenge)}`
    )
    ...
    return res.status(502).json({
      ...
      error_description: "Unable to load the OIDC login challenge.",
    })
  }
}
