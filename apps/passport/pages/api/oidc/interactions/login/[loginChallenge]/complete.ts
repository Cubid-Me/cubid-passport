...
export default async function completeOidcLogin(
  req: NextApiRequest,
  res: NextApiResponse
) {
  if (req.method !== "POST") {
  ...
  try {
    const result = await forwardOidcJson(
      `/interaction/login/${encodeURIComponent(loginChallenge)}/complete`,
      req.body
    )
    ...
    return res.status(502).json({
      ...
      error_description: "Unable to complete the OIDC login challenge.",
    })
  }
}
